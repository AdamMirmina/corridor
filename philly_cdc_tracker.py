"""
Philly CDC/BID leadership + history tracker.

Builds the foundational dataset for the STAR Scholars research question:
...
Run:  py philly_cdc_tracker.py
Out:  output/philly_cdc_dataset.xlsx  and  output/*.csv
"""
...
HEADERS = {"User-Agent": "philly-cdc-tracker (academic research; contact bre34@drexel.edu)"}
...
def write_xlsx(enriched, history_long, leadership_rows):
    ...
    notes = [
        ["Philadelphia CDC / BID leadership + history dataset"],
        ["Generated by philly-cdc-tracker for the STAR Scholars project."],
        [""],
        ["Roster sheet"],
    ...
    path = os.path.join(OUT, "philly_cdc_dataset.xlsx")
    wb.save(path)
    print(f"\nWrote {path}")
